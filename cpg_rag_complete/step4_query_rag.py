...
# Handle different LangChain versions for Document import
try:
...
def main():
    ...
    parser.add_argument("--export", "-e", choices=["json", "md", "markdown"], help="Export format (json, md, or markdown)")
    args = parser.parse_args()
    
    # Suppress INFO logging when exporting (only show WARNING and above)
    if args.export:
        logging.getLogger().setLevel(logging.WARNING)
        logger.setLevel(logging.WARNING)
        # Also suppress langchain and chromadb logging
    ...
    if args.query:
        ...
        # Always print results to stdout (for app display)
        if args.export in ["md", "markdown"]:
            # Print markdown format for display
            print("\n" + "="*70)
            ...
            if sources:
                print("\n## Sources\n")
                for s in sources[:20]:
                    print(f"- **{s.get('function', 'unknown')}** ({s.get('file', 'unknown')}:{s.get('line', '?')})")
                    if s.get('code'):
                        print(f"  ```python\n  {s.get('code')[:200]}...\n  ```")
        elif args.export == "json":
            # Print JSON format for display
            export_data = {
                "query": args.query,
                "type": args.type,
                "answer": answer,
            ...
            # Print to console (default format)
            print("\n" + "="*70)
            print("ANSWER:")
            print("="*70)
            print(answer)
        ...
        # Export to file if requested (in addition to displaying)
        if args.export:
            ...
            if args.export in ["md", "markdown"]:
                # Export as markdown
                output_file = Config.OUTPUT_DIR / f"analysis_{timestamp}.md"
                with open(output_file, 'w', encoding='utf-8') as f:
                    ...
                    if sources:
                        f.write("## Sources\n\n")
                        for s in sources[:20]:
                            f.write(f"- **{s.get('function', 'unknown')}** ({s.get('file', 'unknown')}:{s.get('line', '?')})\n")
                            if s.get('code'):
                                f.write(f"  ```python\n  {s.get('code')[:200]}...\n  ```\n")
                print(f"\n✅ Results also exported to {output_file}")
            else:  # json
                # Export as JSON
